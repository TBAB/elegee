...
<style scoped>
  .level-board { position: relative; margin: 80px auto 20px;}
  .level-block { position: absolute;}
  .random-board { margin: 8px 20px 0px 20px; height: 100px; display: flex;  align-items: center; justify-content: space-between;}
  .random-area { display: flex; justify-content: flex-start;}
  .slot-board { display: flex; justify-content: center; align-items: center; background: url("../assets/bg-bar.png") no-repeat; background-size: contain; height: 82px; width: 348px; margin: 10px auto;}
  .skill-board { width: 80px; height: 80px; background: url("/src/assets/icon-magic.png") no-repeat center; background-size: contain; position: absolute; right: 10px; top: 10px;}
  .skill-board-0 { background: url("../assets/icon-magic.png") no-repeat center; background-size: contain;}
  .skill-board-1 { background: url("../assets/anim-1.png") no-repeat center; background-size: contain;}
  ...
  .icon-monkey { position: absolute; top: 60px; left: 50%; margin-left: -92px; z-index: 998;}
  .icon-light { position: absolute; top: 0; left: 0; z-index: 997; animation: rotate 3s ease infinite;}
  @keyframes rotate { 0% { transform: rotate(0deg);} 100% { transform: rotate(360deg);}}
  @keyframes scale { 0% { transform: scale(1);} 25% { transform: scale(0.9);} 50% { transform: scale(1.1);} 25% { transform: scale(0.9);} 100% {transform: scale(1);}}
  .bg-black { position: absolute; top: 200px; left: 50%; margin-left: -150px; width: 300px; height: 103px; background-color: black; border-radius: 16px; z-index: 999; display: flex; flex-direction: column; align-items: center; justify-content: center;}
  ...
  .slot-block { margin: 0px 0px; position: relative;}
  .audio-board { display: none;}
  .remove-enter-from,.remove-leave-to { opacity: 0; bottom: 10px;}
  .remove-enter-to,.remove-leave-from { opacity: 1; bottom: 0px;}
  .remove-leave-active { transition: all 0.3s ease;}
  .icon-enter-to,.icon-leave-from { transform: scale(1.2);}
  .icon-leave-active { transition: all 0.1s ease;}
